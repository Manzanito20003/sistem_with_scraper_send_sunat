...
class ProductosDialog(QDialog):
    ...
    def init_ui(self):
        ...
        toolbar.addAction(
            QAction("Actualizar", self, triggered=self.actualizar_producto)
        )
        ...
        self.tabla.setHorizontalHeaderLabels(
            ["ID", "Nombre", "Unidad", "Precio", "IGV", "Total"]
        )
    ...
    def actualizar_producto(self):
        ...
        try:
            ...
            igv = (
                1 if self.tabla.item(row, 4).text().lower() in ["sí", "1", "si"] else 0
            )

            self.controller.actualizar_product(
                self.remitente_id, id_producto, nombre, unidad, precio, igv
            )
    ...
    def borrar_producto(self):
        ...
        try:
            ...
            if row < 0:
                QMessageBox.warning(
                    self, "Advertencia", "Seleccione un producto en la tabla."
                )
                return
